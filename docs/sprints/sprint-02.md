...
O protótipo do front-end utiliza dados locais em memória para demonstrar os fluxos da aplicação. A integração definitiva com persistência e os serviços do back-end será realizada nas etapas posteriores do projeto.

Dessa forma, a Sprint 2 não ficou restrita à documentação: houve evolução tanto na camada de apresentação quanto na camada de processamento da aplicação.
...
Para verificar o protótipo do front-end, acessar a pasta `front-end` e abrir o arquivo `index.html` em um navegador.
...
| `#7`  | `Criar histórias de usuário/casos`                  | `@raissafernadesouza @liiafag`      | `Histórias vinculadas aos requisitos`                   | Concluído |
